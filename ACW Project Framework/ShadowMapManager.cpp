...
bool ShadowMapManager::GenerateShadowMapResources(
...
)
{
	...
	{
		result = m_renderToTextures[i]->RenderObjectsToTexture(
			deviceContext, 
			depthStencilView, 
			pointLightList[i]->GetLightViewMatrix(), 
			pointLightList[i]->GetLightProjectionMatrix(),
			pointLightList, 
			gameObjects, 
			cameraPosition);
