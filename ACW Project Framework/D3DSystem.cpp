	...
	m_graphics(nullptr) 
{
	...
	m_graphics = new D3DApplication(screenWidth, screenHeight, m_hwnd);
...
bool D3DSystem::Frame() {
	...
	if (m_input->IsKeyDown(VK_CONTROL))
	{
		...
		{
			m_graphics->GetCamera()->AddPositionZ(0.5f);
		}
		...
		{
			m_graphics->GetCamera()->AddPositionX(0.5f);
		}
		...
		{
			m_graphics->GetCamera()->AddPositionY(0.5f);
		}
